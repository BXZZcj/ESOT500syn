...
from .motion.camera import CAMERA_MOTION_PATTERNS
...
def run(config: dict):
    ...
    env_kwargs = {
        "robot_uids": "none", "sensor_configs": {"shader_pack": cam_cfg.get("shader_pack", "default"), "base_camera": {"width": cam_cfg['width'], "height": cam_cfg['height']}},
        "custom_asset_config": asset_cfg, "distractor_assets_config": distractor_cfg, "lighting_config": light_cfg,
        "sim_config": {"control_freq": sim_cfg.get('control_freq', 30), "sim_freq": sim_cfg.get('sim_freq', 300)}
    }
    if env_cfg['env_type'] == "RoboCasa": env_kwargs.update(scene_cfg['robocasa_params'])
    ...
    cam_sensor, initial_cam_pose = env.unwrapped._sensors['base_camera'], env.unwrapped._sensors['base_camera'].camera.get_local_pose()
    ...
    amodal_verts, amodal_faces = (load_mesh_vertices_faces(asset_cfg["visual_filepath"]) if out_cfg.get("save_annotations") and asset_cfg.get("enable") else (None, None))
    ...
    try:
        for step in range(sim_cfg['num_steps']):
            ...
            if "Color" not in sensor_data: continue
            ...
            if out_cfg.get("save_annotations") and asset_id is not None and "Segmentation" in sensor_data:
                ...
                if bbox_from_mask(modal_mask):
                    ...
                    t_cam, q_cam_wxyz = pose_in_cam.p.cpu().numpy().flatten(), pose_in_cam.q.cpu().numpy().flatten()
                    if amodal_verts is not None:
                        ...
                        if bbox_from_mask(amodal_mask):
                            ...
                            all_frames.append({"rgb_path": f"rgb/rgb_{step:04d}.png", "modal_mask_path": f"modal_mask/modal_mask_{step:04d}.png", "amodal_mask_path": f"amodal_mask/amodal_mask_{step:04d}.png", "bbox_modal_xywh": bbox_from_mask(modal_mask), "bbox_amodal_xywh": bbox_from_mask(amodal_mask), "class": asset_cfg.get("name"), "pose_in_camera": {"position": t_cam.tolist(), "orientation_wxyz": q_cam_wxyz.tolist()}})
            if (step + 1) % 50 == 0 or step == sim_cfg['num_steps'] - 1: print(f"  ... simulating and saving frame {step+1}/{sim_cfg['num_steps']}")
    finally:
        if out_cfg.get("save_annotations") and all_frames:
            with open(output_dir / "annotations.json", 'w') as f: json.dump({"camera_intrinsics": intrinsic_matrix.tolist(), "frames": all_frames}, f, indent=4)
